...
function DataConverter(nodeId) {
  ...
  this.outputDataType         = "jsonArrayRows";
...
DataConverter.prototype.create = function(w,h) {
  ...
  this.inputHeader = $('<div class="groupHeader" id="inputHeader"><p class="groupHeadline">复制Excel中的数据，然后粘贴到下面文本框中</p></div>');
  this.inputwrap = $('<div style="padding:10px;"></div>');
  this.inputoutwrap = $('<div style="padding:10px;"></div>');
  this.inputTextArea = $('<textarea class="textInputs" id="dataInput"></textarea>');
  var outputHeaderText = '<div class="groupHeader" id="inputHeader"><p class="groupHeadline">输出 <select name="Data Types" id="dataSelector" style="display:none;">';
  ...
  this.inputwrap.append(this.inputTextArea);
  this.node.append(this.inputwrap);
  this.node.append(this.outputHeader);
  this.inputoutwrap.append(this.outputTextArea);
  this.node.append(this.inputoutwrap);
...
DataConverter.prototype.resize = function(w,h) {
  ...
  this.inputTextArea.css({width:'100%',height:paneHeight});
  this.outputTextArea.css({width: '100%', height:paneHeight});
